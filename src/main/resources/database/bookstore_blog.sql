...
CREATE TABLE `blog` (
  ...
  KEY `FK_Blog_Media_idx` (`thumbnail`),
  CONSTRAINT `FK_Blog_Media` FOREIGN KEY (`thumbnail`) REFERENCES `media` (`mediaID`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_Blog_User` FOREIGN KEY (`author`) REFERENCES `user` (`userID`) ON UPDATE CASCADE
